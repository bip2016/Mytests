...
if  /usr/sbin/ifplugstatus -a |grep -E 'ppp0'  ;
  then
  {
   echo -en "${GREEN}✔ppp0-интерфейс  поднят!${NORMAL}\n"
    if tcpdump -i ppp0 -l | grep 'isakmp' ;
      then
      {
          echo -en "${GREEN}✔IPSEC-шифрование работает!${NORMAL}\n"
      }
      else
      {
        echo -en "${RED}✘IPSEC-шифрование не работает!${NORMAL}\n"
      }
    fi;
  }
 else
  {
   echo -en "${RED}✘ppp0-интерфейс не поднят!${NORMAL}\n"
  }
 fi;
